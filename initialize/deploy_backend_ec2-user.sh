...
function setup_env() {
    ...
    sudo cat > /home/${DEPLOY_USER}/.env << EOF
...
function create_launch_script () {
    ...
    sudo cat > /home/${DEPLOY_USER}/launch.sh <<EOF
    #!/bin/bash
    cd /home/${DEPLOY_USER}/demo_rest
    source /home/${DEPLOY_USER}/.env
    source /home/${DEPLOY_USER}/venv/bin/activate
    pip show flask
    /home/${DEPLOY_USER}/venv/bin/gunicorn -b :8831 run:application
EOF
    sudo chmod 744 /home/${DEPLOY_USER}/launch.sh
    echo ====== Ensuring script is executable =======
    ls -la /home/${DEPLOY_USER}/launch.sh
}

function configure_startup_service () {
    printf "***************************************************\n\t\tConfiguring startup service \n***************************************************\n"

    sudo bash -c 'cat > /etc/systemd/system/demo_rest.service <<EOF
    [Unit]
    Description=demo_rest startup service
    After=network.target

    [Service]
    User=${DEPLOY_USER}
    ExecStart=/bin/bash /home/${DEPLOY_USER}/launch.sh
...
if [ ! -f ${LOG_FILE_FULL} ]; then
    ...
    sudo chown ${DEPLOY_USER} ${LOG_FILE_FULL}
    sudo chgrp ${DEPLOY_USER} ${LOG_FILE_FULL}
fi
...
function launch_app() {
    printf "***************************************************\n\t\tServing the App \n***************************************************\n"
    sudo bash /home/${DEPLOY_USER}/launch.sh
}
